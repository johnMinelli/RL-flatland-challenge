...
def main(argv):
    ...
    env = RailEnv(width=x_dim,
                  height=y_dim,
                  rail_generator=sparse_rail_generator(max_num_cities=3, # Number of cities in map (where train stations are)
                                                       seed=1,  # Random seed
                                                       grid_mode=False,
    ...
    state_size = num_features_per_node * nr_nodes # 231
    ...
    agent = DQNPolicy((state_size,), action_size)
    ...
    for trials in range(1, n_trials + 1):
        ...
        for a in range(n_agents):
            ...
            if obs[a]:
                ...
                agent_obs[a] = obs[a]
                agent_obs_buffer[a] = copy.deepcopy(agent_obs[a])
        ...
        for step in range(max_steps):
            ...
            for a in range(n_agents):
                if info['action_required'][a]:
                    ...
                    update_values = True
                    print(agent_obs[a].get())
                    action = agent.act(agent_obs[a].values())
